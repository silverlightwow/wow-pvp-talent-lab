...
def rank_effects(dump, talent):
    ...
    for line in spell.raw.splitlines():
        entry = re.match(
            r"^Talent Entry\s*:\s*(.*?)\s*\[tree=([^,]+),.*max_rank=(\d+)", line
        )
        if entry:
            active = (
                (
                    entry[1] == "Generic"
                    or talent["spec_name"] in [s.strip() for s in entry[1].split(",")]
                )
            ...
            continue
        rule = re.match(
            r"^\s+: Effect#(\d+) \[op=(set|mul|add), values=\(([^)]+)\)\]", line
        )
        if active and rule:
            index = int(rule[1])
            ...
            if index in rules and rules[index] != (rule[2], values):
                raise ValueError(
                    f"Conflicting rank rules: {spell.spell_id}, effect {index}"
                )
    ...
    for index, (op, values) in rules.items():
        ...
        result.append(
            dict(
                ...
                base_value=base,
                values=[
                    v if op == "set" else base * v if op == "mul" else base + v
                    for v in values
                ],
                simc_reference_contexts=list(
                    simc.effect_reference_contexts(dump, spell.spell_id, index)
                )
                + [
                    line
                    for line in simc._player_text_sections(spell.raw)
                    if re.search(
                        r"\$(?:"
                        + str(spell.spell_id)
                        + r")?[mMwW]"
...
def rows_at_rank(rows, rules, rank, spell_id):
    ...
    for original in rows:
        row = dict(original)
        if (
            row.get("source_spell_id", row.get("spell_id", spell_id)) == spell_id
            and row.get("effect_index") in values
        ):
...
def rank_expressions(dump, talent, rules):
    ...
    spec_auras = {
        s.spell_id: s.name == f"{talent['spec_name']} {talent['class_name']}"
        for s in dump.spells.values()
    ...
    variables = dict(
        re.findall(
            r"^\s*\$([A-Za-z_]\w*)=(.*)$", spell.raw.split("Variables", 1)[-1], re.M
        )
    )
    ...
    # Keep character-dependent formulas symbolic, as the tooltip service does.
    # The ranked coefficient inside the formula still has an exact value.
    def symbolic_formula(match):
        formula = match[1]
        if not re.search(r"\$(?:AP|SP|@versadmg)\b", formula):
            return match[0]
        formula = (
            formula.replace("$AP", "Attack Power")
            .replace("$SP", "Spell Power")
            .replace("$@versadmg", "Versatility")
        )
        formula = re.sub(r"\s*([+*/-])\s*", r" \1 ", formula)
        return "[" + formula + "]"

    text = re.sub(r"\$\{([^{}]+)\}", symbolic_formula, text)
    ...
    for position, match in enumerate(tokens):
        ...
        indexes = {
            int(m[2])
            for m in references
            if (not m[1] or int(m[1]) == spell.spell_id) and int(m[2]) in by_index
        }
        if not indexes:
            continue
        # Some client formulas have a surplus closing parenthesis (Improved Fade).
        if formula.count(")") == formula.count("(") + 1 and formula.endswith(")"):
            formula = formula[:-1]
        coefficient = False
        ...
        def evaluate(rank):
            ...
            def resolve(m):
                ...
                if effect is None:
                    raise ValueError("Missing referenced rank effect")
                if sid == spell.spell_id and index in by_index:
                    value = (
                        by_index[index]["base_value"]
                        if rank == 0
                        else by_index[index]["values"][rank - 1]
                    )
                elif effect.sp_coefficient or effect.ap_coefficient:
                    coefficient = True
                    value = (effect.sp_coefficient or effect.ap_coefficient) * 100
                else:
            ...
            def stack_count(match):
                referenced = dump.spells.get(int(match[1]))
                stacks = (
                    re.search(r"^Stacks\s*:\s*(\d+) maximum", referenced.raw, re.M)
                    if referenced
                    else None
                )
                if stacks is None:
                    raise ValueError("Missing referenced stack count")
                return stacks[1]

            value = re.sub(r"\$(\d+)[uU]\b", stack_count, value)
            return abs(_arithmetic(value.replace("$abs", "abs").strip()))

        try:
            old = evaluate(0)
            values = [evaluate(r) for r in range(1, int(talent["max_ranks"]) + 1)]
        except (ValueError, SyntaxError, ZeroDivisionError) as exc:
            raise ValueError(
                f"Unsupported rank expression for {spell.spell_id}: {formula}"
            ) from exc
        index = min(indexes)
        ...
        before = re.split(
            r"[\[\]\n$<>]",
            text[tokens[position - 1].end() if position else 0 : match.start()],
        )[-1][-70:]
        before = (
            re.sub(r"^[a-zA-Z]+[0-9]*%?\s+", "", before)
            if "$"
            in text[tokens[position - 1].end() if position else 0 : match.start()]
            else before
        )
        after = re.split(
            r"[\[\]\n$<>]",
        ...
        expressions.append(
            dict(
                ...
                precision=(
                    int(re.search(r"\}\.(\d+)$", match[0])[1])
                    if re.search(r"\}\.(\d+)$", match[0])
                    else 0 if match[1] is not None and not coefficient else 4
                ),
...
def render_rank(text, source, rank, *, spec_name, spec_names):
    ...
    for expression in source["expressions"]:
        old = expression["base_value"]
        new = expression["values"][rank - 1]
        if old == new:
            continue
        ...
        matches = (
            list(re.finditer(pattern, text, re.I))
            if len(re.findall(r"[A-Za-z]{3,}", before + after)) >= 2
            else []
        )
        if len(matches) > 1:
        ...
        if not matches:
            candidates = list(
                re.finditer(r"(?<![\d.])([+-]?\d+(?:\.\d+)?)(?![\d.])", text)
            )
            equal = [m for m in candidates if abs(float(m[1]) - old) < 1e-5]
            matches = tooltip_renderer._select_reference_context_matches(
                text,
                equal,
                reference_contexts=[expression["context"]],
                effect_index=expression["effect_index"],
            )
            if not matches and equal:
                chosen = tooltip_renderer._select_reference_context_match(
                    text,
                    equal,
                    reference_contexts=[expression["context"]],
                    effect_index=expression["effect_index"],
        ...
        if not matches:
            diagnostics.append(
                dict(
                    status="RANK_EXPRESSION_NOT_VISIBLE",
                    effect_index=expression["effect_index"],
                    old=old,
                    new=new,
                )
        ...
        for match in matches:
            ...
            value = (
                new
                if abs(actual - old) < 0.00001
                or (actual == round(old) and abs(actual - old) < 0.5)
                else actual * new / old if old else new
            )
            value = round(value, expression["precision"])
            key = match.span(1)
            replacement = (
                f"{value:.4f}".rstrip("0").rstrip(".") if value % 1 else str(int(value))
            )
    ...
    for switch in source.get("switches", []):
        old = switch["options"][int(switch["base"]) - 1]
        new = switch["options"][int(switch["values"][rank - 1]) - 1]
        text = text.replace(
            switch["before"] + old + switch["after"],
            switch["before"] + new + switch["after"],
        )

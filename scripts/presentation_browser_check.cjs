...
(async () => {
 ...
 try {
  for(const width of [1440,390,320]) {
   ...
   assert.ok(await page.locator('#spec-aura').count());
   assert.ok(await page.locator('#examples').count());
   assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false,'Docs horizontal overflow');
   await shot(page,`docs-${width}`);
   await page.locator('.docs-nav a[href="#spec-aura"]').click();
   assert.ok(page.url().endsWith('#spec-aura'));
